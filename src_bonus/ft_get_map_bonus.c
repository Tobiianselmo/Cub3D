...
/*   Updated: 2024/12/27 13:38:14 by dparada          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_save_coor(t_cub *g, char *line, int *flag)
{
	if (ft_strcmp(line, "\n") && ft_strlen(line)
		&& (ft_strncmp(line, "NO", ft_first_char(line, ' '))
			...
			&& ft_strncmp(line, "D", ft_first_char(line, ' '))
...
void	ft_maps(t_cub *game, char *aux, char *result)
{
	...
	if (result)
		free(result);
	if (flag)
		ft_msj_error(game, 1, "Unrecognized line.");
}
